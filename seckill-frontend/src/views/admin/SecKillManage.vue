        ...
        <template #table>
            <el-scrollbar>
                <el-table :data="secKillList.items" style="width: 100%">
                    ...
                    <el-table-column label="商品描述" prop="description" width="200" />
                    <el-table-column label="秒杀价" prop="secKillPrice" width="120">
                        <template #default="{ row }">
                            <div class="price">
                                <div><el-text>{{ row.secKillPrice }}元/{{ row.unit }}</el-text></div>
                                <div><el-text size="small" tag="del">{{ row.price }}元/{{ row.unit }}</el-text></div>
                            </div>
                        </template>
                    </el-table-column>
                    <el-table-column label="秒杀库存" prop="secKillStock" width="80" />
                    <el-table-column label="限购数" prop="limitNum" width="80" />
                    <el-table-column label="状态" prop="status" width="120">
                        <template #default="{ row }">
                            <el-tag type="primary" v-if="row.status === 1">未开始</el-tag>
                            <el-tag type="warning" v-else-if="row.status === 2">进行中</el-tag>
                            <el-tag type="info" v-else-if="row.status === 0">已结束</el-tag>
                        </template>
                    </el-table-column>
